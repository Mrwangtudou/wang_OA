...
<script type="text/javascript">
	$(function() {
		 ...
		 $.post("${ctx}/identity/user/ajaxLoadDeptAndJob.jspx",function(data){
			  ...
			  $.each(depts,function(i,item){
				 ...
				 // $("<option>").val(item.id).html(item.name).appendTo("#deptSelect");
				  $("<option>").val(this.id).html(this.name).appendTo("#deptSelect");
			  })
		...
		$("#userId").blur(function(){
			...
			//获取用户输入的信息
			//var zhangHao = $("#userId").val();
			var zhangHao = this.value;
			if(!/^[0-9a-zA-Z]{5,10}$/.test(zhangHao)){
				//this.value = "";//清空在用户输入的信息
				$("#userId").val("");//清空在用户输入的信息
				$("#userId").focus();
				 $.messager.alert('错误提示',"您输入的账号格式不正确，请重新输入！",'warning');
			}else{
				//用户输入的账号语法正确，发送异步请求继续判断账号是否存在
				 $.post("${ctx}/identity/user/userExistOrNot.jspx",{"userId":zhangHao},function(data,status){
                      //status=="success"：后台相应成功
                      if(status=="success"){
                    	  if(data){
        		    		  $("#userId").val("");//清空在用户输入的信息
        					  $.messager.alert('错误提示',data,'warning');
                    	  }
                      }
