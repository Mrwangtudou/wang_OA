 ...
 function refreshMenu(){
		...
		success:function(data){
             $.each(data,function(){
            	//第一个参数：id(必须唯一)  第二个参数：pid(父节点id)  第三个参数：name（节点名字)  第四个参数：url（跳转地址) 第五个参数：title（标题)  第六个参数：target(跳转的目标位置))
            	 d.add(this.code,this.pCode,this.name,'${ctx}/identity/module/selectModuleByParentCode.jspx?code='+this.code,this.name,"rightFrame");
             })			
    ...
    <body class="easyui-layout" style="width:100%;height:100%;">
			...
			<div data-options="region:'center'" title="模块菜单"  >
			     <!-- 展示当前模块下的子模块  -->
			     <iframe src="${ctx}/identity/module/selectModuleByParentCode.jspx?code=" frameborder="0" id="sonModules" name="rightFrame"  width="100%" height="100%" ></iframe>
			</div>
	</body>
</html>
